...
void GPIO_ISR(void)
{
  ...
  i = M_PSP_READ_REGISTER_32(RGPIO_INTS);
  i = i >> 16;
  if((i & 0x1) == 1){
    count = 0;
  ...
  i = M_PSP_READ_REGISTER_32(RGPIO2_INTS);
  if((i & 0x1) == 1){
    if (delay == 0x5FFFF) delay = 0x5FFFFF;
    else if (delay == 0x5FFFFF) delay = 0x5FFFF;
