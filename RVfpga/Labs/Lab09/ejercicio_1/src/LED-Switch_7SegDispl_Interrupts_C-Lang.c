...
// Se agrega el modulo de PTC_ISR
void PTC_ISR(void)
{
  M_PSP_WRITE_REGISTER_32(SegDig_ADDR, SegDisplCount);
  SegDisplCount++;
  M_PSP_WRITE_REGISTER_32(RPTC_CNTR, 0x0); 
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0x31); //Limpiamos el GPIO

  bspClearExtInterrupt(3); //Detenemos la generacion de una interrupcion externa
}

// Siguiendo la logica de GPIO_Initialization
void PTC_Initialization(void)
{
  M_PSP_WRITE_REGISTER_32(RPTC_LRC, 0xFFFF); //Configuramos el LRC
  ////Configuracion del counter y control
  M_PSP_WRITE_REGISTER_32(RPTC_CNTR, 0x0); 
